...
int disasm(uint32_t addr, uint16_t insword, char *result)
{
	...
	// 0011nnnnmmmm1100 "add Rm,Rn"
		...
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "add r%d,r%d", m, n);
	}
	...
	// 0011nnnnmmmm1110 "addc Rm,Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "addc r%d,r%d", m, n);
	}
	...
	// 0011nnnnmmmm1111 "addv Rm,Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "addv r%d,r%d", m, n);
	}

	// 11001001iiiiiiii "and #imm,r0"
	else if((insword & 0xff00) == 0xc900) {
		int16_t i = int8(insword & 0xff);
		sprintf(result, "and #%d,r0", i);
	}
	...
	// 0011nnnnmmmm0000 "cmp/eq Rm,Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "cmp/eq r%d,r%d", m, n);
	}
	...
	// 0011nnnnmmmm0011 "cmp/ge Rm,Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "cmp/ge r%d,r%d", m, n);
	}
	...
	// 0011nnnnmmmm0111 "cmp/gt Rm,Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "cmp/gt r%d,r%d", m, n);
	}
	...
	// 0011nnnnmmmm0110 "cmp/hi Rm,Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "cmp/hi r%d,r%d", m, n);
	}
	...
	// 0010nnnnmmmm1100 "cmp/str Rm,Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "cmp/str r%d,r%d", m, n);
	}
	...
	// 0010nnnnmmmm0111 "div0s Rm,Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "div0s r%d,r%d", m, n);
	}

	// 0000000000011001 "div0u"
	else if(insword == 0x19) {
		sprintf(result, "div0u");
	}
	...
	// 0011nnnnmmmm0100 "div1 Rm,Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "div1 r%d,r%d", m, n);
	}
	...
	// 0011nnnnmmmm1101 "dmuls.l Rm,Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "dmuls.l r%d,r%d", m, n);
	}
	...
	// 0011nnnnmmmm0101 "dmulu.l Rm,Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "dmulu.l r%d,r%d", m, n);
	}

	// 0100nnnn00010000 "dt Rn"
	else if((insword & 0xf0ff) == 0x4010) {
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "dt r%d", n);
	}
	...
	// 0110nnnnmmmm1110 "exts.b Rm,Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "exts.b r%d,r%d", m, n);
	}
	...
	// 0110nnnnmmmm1111 "exts.w Rm,Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "exts.w r%d,r%d", m, n);
	}
	...
	// 0110nnnnmmmm1100 "extu.b Rm,Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "extu.b r%d,r%d", m, n);
	}
	...
	// 0110nnnnmmmm1101 "extu.w Rm,Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "extu.w r%d,r%d", m, n);
	}

	// 1111nnn001011101 "fabs DRn"
	else if((insword & 0xf1ff) == 0xf05d) {
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "fabs fr%d", n);
	}
	...
	// 1111nnn0mmmm0000 "fadd DRm,DRn"
		...
		uint16_t m = (insword & 0xF0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "fadd fr%d,fr%d", m, n);
	}
	...
	// 1111nnn0mmmm0100 "fcmp/eq DRm,DRn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "fcmp/eq fr%d,fr%d", m, n);
	}
	...
	// 1111nnn0mmm00101 "fcmp/gt DRm,DRn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "fcmp/gt fr%d,fr%d", m, n);
	}
	...
	// 1111mmm010111101 "fcnvds DRm,fpul"
		...
		uint16_t m = (insword & 0xf00)>>8;
		sprintf(result, "fcnvds dr%d,fpul", m);
	}

	// 1111nnn010101101 "fcnvsd fpul,DRn"
	else if((insword & 0xf1ff) == 0xf0ad) {
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "fcnvsd fpul,dr%d", n);
	}
	...
	// 1111nnn0mmmm0011 "fdiv DRm,DRn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "fdiv fr%d,fr%d", m, n);
	}
	...
	// 1111nnnnmmmm1110 "fmac fr0,FRm,FRn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "fmac fr0,fr%d,fr%d", m, n);
	}
	...
	// 1111nnn0mmm01100 "fmov DRm,DRn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "fmov fr%d,fr%d", m, n);
	}
	...
	// 1111nnn0mmm11100 "fmov XDm,DRn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "fmov XD%d,fr%d", m, n);
	}
	...
	// 1111nnn0mmmm0110 "fmov @(r0,Rm),DRn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "fmov @(r0,r%d),fr%d", m, n);
	}
	...
	// 1111nnn0mmmm1001 "fmov @Rm+,DRn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "fmov @r%d+,fr%d", m, n);
	}
	...
	// 1111nnnnmmm00111 "fmov DRm,@(r0,Rn)"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "fmov fr%d,@(r0,r%d)", m, n);
	}
	...
	// 1111nnnnmmm01011 "fmov DRm,@-Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "fmov fr%d,@-r%d", m, n);
	}
	...
	// 1111nnnnmmmm0110 "fmov.s @(r0,Rm),FRn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "fmov.s @(r0,r%d),fr%d", m, n);
	}
	...
	// 1111nnnnmmmm1001 "fmov.s @Rm+,FRn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "fmov.s @r%d+,fr%d", m, n);
	}
	...
	// 1111nnnnmmmm0111 "fmov.s FRm,@(r0,Rn)"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "fmov.s fr%d,@(r0,r%d)", m, n);
	}
	...
	// 1111nnnnmmmm1011 "fmov.s FRm,@-Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "fmov.s fr%d,@-r%d", m, n);
	}
	...
	// 1111nnn0mmmm0010 "fmul DRm,DRn"
		...
		uint16_t m = (insword & 0xF0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "fmul fr%d,fr%d", m, n);
	}
	...
	// 1111nnnn00001101 "fsts fpul,FRn"
		...
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "fsts fpul,fr%d", n);
	}
	...
	// 1111nnn0mmmm0001 "fsub DRm,DRn"
		...
		uint16_t m = (insword & 0xF0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "fsub fr%d,fr%d", m, n);
	}
	...
	// 1111mmm000111101 "ftrc DRm,fpul"
		...
		uint16_t m = (insword & 0xf00)>>8;
		sprintf(result, "ftrc fr%d,fpul", m);
	}
	...
	// 0100mmmm00111010 "ldc Rm,sgr"
		...
		uint16_t m = (insword & 0xf00)>>8;
		sprintf(result, "ldc r%d,sgr", m);
	}

	// 0100mmmm00101110 "ldc Rm,vbr"
	else if((insword & 0xf0ff) == 0x402e) {
		uint16_t m = (insword & 0xf00)>>8;
		sprintf(result, "ldc r%d,vbr", m);
	}
	...
	// 0100mmmmnnnn1110
		...
		if(n < 5) {
			...
			sprintf(result, "ldc r%d,%s", m, lookup[n]);
		}
		else
			sprintf(result, "ldc r%d,r%d_bank", m, n);
	}
	...
	// 0100mmmm00010111 "ldc.l @Rm+,gbr"
		...
		uint16_t m = (insword & 0xf00)>>8;
		sprintf(result, "ldc.l @r%d+,gbr", m);
	}
	...
	// 0100mmmm00110110 "ldc.l @Rm+,sgr"
		...
		uint16_t m = (insword & 0xf00)>>8;
		sprintf(result, "ldc.l @r%d+,sgr", m);
	}

	// 0100mmmm00100111 "ldc.l @Rm+,vbr"
	else if((insword & 0xf0ff) == 0x4027) {
		uint16_t m = (insword & 0xf00)>>8;
		sprintf(result, "ldc.l @r%d+,vbr", m);
	}
	...
	// 0100mmmmnnnn0111 "ldc.l @Rm+,vbr"
		...
		if(n < 5) {
			const char *lookup[5] = {"sr","gbr","vbr","ssr","spc"};
			sprintf(result, "ldc.l @r%d+,%s", m, lookup[n]);
		}
		else
			sprintf(result, "ldc.l @r%d+,r%d_bank", m, n);
	}
	...
	// 0000nnnnmmmm1111 "mac.l @Rm+,@Rn+"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "mac.l @r%d+,@r%d+", m, n);
	}
	...
	// 0100nnnnmmmm1111 "mac.w @Rm+,@Rn+"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "mac.w @r%d+,@r%d+", m, n);
	}
	...
	// 0110nnnnmmmm0011 "mov Rm,Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "mov r%d,r%d", m, n);
	}
	...
	// 10000100mmmmdddd "mov.b @(disp,Rm),r0"
		...
		uint16_t m = (insword & 0xf0)>>4;
		sprintf(result, "mov.b @(%d,r%d),r0", d, m);
	}
	...
	// 0110nnnnmmmm0100 "mov.b @Rm+,Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "mov.b @r%d+,r%d", m, n);
	}
	...
	// 11000000dddddddd "mov.b r0,@(disp,gbr)"
		...
		uint16_t d = insword & 0xff;
		sprintf(result, "mov.b r0,@(%d,gbr)", d);
	}
	...
	// 10000000nnnndddd "mov.b r0,@(disp,Rn)"
		...
		uint16_t d = insword & 0xf;
		uint16_t n = (insword & 0xf0)>>4;
		sprintf(result, "mov.b r0,@(%d,r%d)", d, n);
	}
	...
	// 0000nnnnmmmm0100 "mov.b Rm,@(r0,Rn)"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "mov.b r%d,@(r0,r%d)", m, n);
	}
	...
	// 0000nnnnmmmm1110 "mov.l @(r0,Rm),Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "mov.l @(r0,r%d),r%d", m, n);
	}

	// 11000110dddddddd "mov.l @(disp,gbr),r0"
	else if((insword & 0xff00) == 0xc600) {
		uint16_t d = insword & 0xff;
		sprintf(result, "mov.l @(%d,gbr),r0", 4*d);
	}
	...
	// 1101nnnndddddddd "mov.l @(disp,PC),Rn"
		...
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "mov.l 0x%016llx,r%d", d, n);
	}
	...
	// 0101nnnnmmmmdddd "mov.l @(disp,Rm),Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "mov.l @(%d,r%d),r%d", 4*d, m, n);
	}
	...
	// 0110nnnnmmmm0110 "mov.l @Rm+,Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "mov.l @r%d+,r%d", m, n);
	}
	...
	// 11000010dddddddd "mov.l r0,@(disp,gbr)"
		...
		uint16_t d = insword & 0xff;
		sprintf(result, "mov.l r0,@(%d,gbr)", 4*d);
	}
	...
	// 0000nnnnmmmm0110 "mov.l Rm,@(r0,Rn)"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "mov.l r%d,@(r0,r%d)", m, n);
	}
	...
	// 0001nnnnmmmmdddd "mov.l Rm,@(disp,Rn)"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "mov.l r%d,@(%d,r%d)", m, 4*d, n);
	}
	...
	// 0000nnnnmmmm1101 "mov.w @(r0,Rm),Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "mov.w @(r0,r%d),r%d", m, n);
	}

	// 11000101dddddddd "mov.w @(disp,gbr),r0"
	else if((insword & 0xff00) == 0xc500) {
		uint16_t d = insword & 0xff;
		sprintf(result, "mov.w @(%d,gbr),r0", 2*d);
	}
	...
	// 1001nnnndddddddd "mov.w @(disp,PC),Rn"
		...
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "mov.w 0x%016x,r%d", d, n);
	}
	...
	// 10000101mmmmdddd "mov.w @(disp,Rm),r0"
		...
		uint16_t d = insword & 0xf;
		uint16_t m = (insword & 0xf0)>>4;
		sprintf(result, "mov.w @(%d,r%d),r0", 2*d,m);
	}
	...
	// 0110nnnnmmmm0101 "mov.w @Rm+,Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "mov.w @r%d+,r%d", m, n);
	}
	...
	// 11000001dddddddd "mov.w r0,@(disp,gbr)"
		...
		uint16_t d = insword & 0xff;
		sprintf(result, "mov.w r0,@(%d,gbr)", 2*d);
	}
	...
	// 10000001nnnndddd "mov.w r0,@(disp,Rn)"
		...
		uint16_t d = insword & 0xf;
		uint16_t n = (insword & 0xf0)>>4;
		sprintf(result, "mov.w r0,@(%d,r%d)", 2*d, n);
	}
	...
	// 0000nnnnmmmm0101 "mov.w Rm,@(r0,Rn)"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "mov.w r%d,@(r0,r%d)", m, n);
	}
	...
	// 0000nnnnmmmm0111 "mul.l Rm,Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "mul.l r%d,r%d", m, n);
	}
	...
	// 0010nnnnmmmm1111 "muls.w Rm,Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "muls.w r%d,r%d", m, n);
	}
	...
	// 0010nnnnmmmm1110 "mulu.w Rm,Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "mulu.w r%d,r%d", m, n);
	}
	...
	// 0110nnnnmmmm1011 "neg Rm,Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "neg r%d,r%d", m, n);
	}
	...
	// 0110nnnnmmmm1010 "negc Rm,Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "negc r%d,r%d", m, n);
	}
	...
	// 0000nnnn0mmm0010 "stc Rm_bank,Rn"
		...
		if(m < 5) {
			const char *lookup[5] = {"sr","gbr","vbr","ssr","spc"};
			sprintf(result, "stc %s,r%d", lookup[m], n);
		}
		else
			sprintf(result, "stc r%d_bank,r%d", m, n);
	}
	...
	// 0100nnnn11110010 "stc.l dbr,@-Rn"
		...
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "stc.l dbr,@-r%d", n);
	}

	// 0100nnnn00010011 "stc.l gbr,@-Rn"
	else if((insword & 0xf0ff) == 0x4013) {
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "stc.l gbr,@-r%d", n);
	}
	...
	// 0100nnnn1mmm0011 "stc.l Rm_bank,@-Rn"
		...
		uint16_t m = (insword & 0x70)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "stc.l r%d_bank,@-r%d", m, n);
	}
	...
	// 0100nnnn0mmm0011 "stc.l Rm_bank,@-Rn"
		...
		if(m < 5) {
			const char *lookup[5] = {"sr","gbr","vbr","ssr","spc"};
			sprintf(result, "stc.l %s,@-r%d", lookup[m], n);
		}
		else
			sprintf(result, "stc.l r%d_bank,@-r%d", m, n);
	}
	...
	// 0011nnnnmmmm1000 "sub Rm,Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "sub r%d,r%d", m, n);
	}
	...
	// 0011nnnnmmmm1010 "subc Rm,Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "subc r%d,r%d", m, n);
	}
	...
	// 0011nnnnmmmm1011 "subv Rm,Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "subv r%d,r%d", m, n);
	}
	...
	// 0110nnnnmmmm1000 "swap.b Rm,Rn"
		...
		uint16_t m = (insword & 0xf0)>>4;
		uint16_t n = (insword & 0xf00)>>8;
		sprintf(result, "swap.b r%d,r%d", m, n);
	}
